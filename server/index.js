...
app.get('/auth/vicare', (req, res) => {
  res.redirect(vicare.buildAuthUrl());
});

app.get('/auth/vicare/callback', async (req, res) => {
  try {
    await vicare.exchangeCodeForToken(req.query.code);
...
app.post('/api/vicare/start', async (req, res) => {
  try {
    const result = await vicare.start();
    const scheduled = coordinator.scheduleTuyaStart();
    res.json({ vicare: result, fancoil: scheduled });
  } catch (err) {
    sendError(res, err);
  }
});
...
app.post('/api/vicare/stop', async (req, res) => {
  try {
    const result = await vicare.stop();
    const scheduled = coordinator.scheduleTuyaStop();
    res.json({ vicare: result, fancoil: scheduled });
  } catch (err) {
    sendError(res, err);
  }
